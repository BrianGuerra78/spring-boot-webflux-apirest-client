...
public class ProductoServiceImpl implements ProductoService {
	...
	public Mono<Producto> findById(String id) {
		...
		/*
		 * .exchange() .flatMap(response -> response.bodyToMono(Producto.class));
		 */
	}
	...
	public Mono<Producto> update(Producto producto, String id) {
		return client.put().uri("/{id}", Collections.singletonMap("id", id)).accept(APPLICATION_JSON)
				.contentType(APPLICATION_JSON).syncBody(producto).retrieve().bodyToMono(Producto.class);
	}
	...
	public Mono<Producto> upload(FilePart file, String id) {
		...
		return client.post().uri("/upload/{id}", Collections.singletonMap("id", id)).contentType(MULTIPART_FORM_DATA)
				.syncBody(parts.build()).retrieve().bodyToMono(Producto.class);
	}
