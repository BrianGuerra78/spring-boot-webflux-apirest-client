...
import com.bolsadeideas.springboot.webflux.client.app.services.ProductoService;
...
public class ProductoHandler {
	...
	public Mono<ServerResponse> ver(ServerRequest request) {
		...
		return errorHandler(
				service.findById(id).flatMap(p -> ServerResponse.ok().contentType(APPLICATION_JSON).syncBody(p))
	...
	public Mono<ServerResponse> crear(ServerRequest request) {
		...
		return producto.flatMap(p -> {
			...
			return service.save(p);
				...
				.contentType(APPLICATION_JSON).syncBody(p)).onErrorResume(error -> {
	...
	public Mono<ServerResponse> editar(ServerRequest request) {
		Mono<Producto> producto = request.bodyToMono(Producto.class);
		String id = request.pathVariable("id");
		return errorHandler(producto.flatMap(p -> service.update(p, id)).flatMap(p -> ServerResponse
				.created(URI.create("/api/client/".concat(p.getId()))).contentType(APPLICATION_JSON).syncBody(p)));
	}
	...
	public Mono<ServerResponse> upload(ServerRequest request) {
		...
		return errorHandler(request.multipartData().map(multipart -> multipart.toSingleValueMap().get("file"))
				...
				.flatMap(p -> ServerResponse.created(URI.create("/api/client/".concat(p.getId())))
						.contentType(APPLICATION_JSON).syncBody(p)));
	}
